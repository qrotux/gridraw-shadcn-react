...
- **`src/slots.tsx`** — the UI seam: `GridComponents` (17 replaceable components), `GridClassNames`, `defaultGridComponents`, `mergeComponents`, `GridUiProvider`/`useGridUi`, and the native `<select>` that backs the `Select` slot. Nothing under `src/` imports `src/ui/*` directly any more except this file and its own defaults.
...
- **Import direction:** `core` ← everything else. `core` imports nothing from `src/` outside itself. `src/ui/*` is reached through `useGridUi()`, not by importing it at a call site; `cn` and `controlClass` are the two utilities still imported directly.
...
- **Every shadcn component the grid renders is a slot.** `GridPage` takes `components` (per-slot, merged over `defaultGridComponents`) and `classNames` (appended through `cn`). A slot is typed by the props the grid actually passes it, never by the full upstream type, so a consumer's component drops in structurally and no `@radix-ui/*` type reaches the public API. Adding a component means adding a slot, not an import from `src/ui/`. `Select` is the one slot with a shape of our own: shadcn's is a five-part Radix composite and the default here is a native `<select>`, which keeps the dependency out.
...
- **Pagination follows `hasPrev`/`hasNext`, not page arithmetic.** The rows response always carries both; `total` is absent on a `skipTotal` grid (the descriptor says so), and only its presence distinguishes that from a counting grid's explicit `"total": 0`. Without a total there is no page count: the label is the page number alone and the row-count line is not rendered. A server predating the flags sends neither, and the page falls back to the old arithmetic so its arrows keep working.
...
- **Column descriptions are optional prose.** The descriptor may carry `description` on the grid and on any column; a column's shows as a tooltip on its table header and as an ⓘ marker in the column picker, after a deliberate two-second delay so a pointer crossing the header sets off nothing. Never make a description load-bearing — a grid without one must read the same.
