...
- **Value inputs coerce to the wire type on every change** (numbers, booleans, ISO datetimes, `[a, b]` for `between`, `string[]` for `in`). The Go backend rejects string-typed numbers and booleans.
